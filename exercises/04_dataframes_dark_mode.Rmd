...
```{r}
nerd[3, 1] # [rows, columns], so 3rd row, first column
```

If we want to get an entire column, you just leave the rows empty:

```{r}
head(nerd[ , 3], 10) # get entire 3rd column
```
...
```{r}
rows_ten_thru_thirteen <- nerd[10:13, ]
rows_ten_thru_thirteen
```

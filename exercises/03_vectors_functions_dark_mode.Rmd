...
```{r}
numbers[c(3, 4)] <- c(222, 333)
numbers
numbers[3:4]<- c(444, 555)
numbers
```
...
```{r}
numbers
numbers[-3] # not permanent, just in-place
numbers
numbers <- numbers[-3] # overwrite original to make permanent
numbers
numbers[-c(3, 4)]
```
...
```{r}
hotdog_women <- hotdog_names[hotdog_sex == "female"]
hotdog_women[hotdog_2019 == max(hotdog_2019)]

hotdog_names[hotdog_2019 == max(hotdog_2019[hotdog_sex == "female"])]
(hotdog_names[hotdog_sex == "female"])[hotdog_2019 == max(hotdog_2019)]
```
...
```{r}
missing_numbers <- c(1, 20, NA, 12, 3, 15, NA, 7)
missing_numbers
```
...
```{r}
min(missing_numbers)
min(missing_numbers, na.rm = TRUE)
max(missing_numbers)
max(missing_numbers, na.rm = TRUE)
```
...
```{r}
summary(missing_numbers)
summary(missing_numbers, na.rm = TRUE)
```
...
```{r}
head(missing_numbers)
head(missing_numbers, na.rm = TRUE)
```

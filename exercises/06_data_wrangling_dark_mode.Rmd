...
nerd_red <- 
  nerd_red %>% 
  filter(
    age >= 18, # could also do age %in% 18:100
    age <= 100,
    married %in% 1:3, 
    voted %in% 1:2, 
    nerdy %in% 1:7)
...
```{r}
nerd_red %>% 
  mutate(age = age + 10) # here you have to use =, the pipe operator ( <- ) does not work
```
...
nerd_red %>% 
  ...
  ungroup() # reverts the rowwise thing which is permanent
```
